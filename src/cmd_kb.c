...
static void kb_cmd_calibrate(app_ctx_t *ctx, int argc, char **argv)
{
   static const char *bool_flags[] = {"check-readiness", NULL};
   opt_parsed_t opts;
   opt_parse(argc, argv, bool_flags, &opts);
   int check = opt_get_flag(&opts, "check-readiness");
   int json_out = ctx && ctx->json_output;

   if (!check)
   {
      fprintf(stderr, "usage: aimee kb calibrate --check-readiness\n");
      return;
   }

   char *raw = kb_client_calibrate_readiness_json();
   if (!raw)
   {
      fprintf(stderr, "calibrate: no response from aimee-kb\n");
      return;
   }
   if (json_out)
   {
   ...
   free(raw);
   cJSON *status = resp ? cJSON_GetObjectItemCaseSensitive(resp, "status") : NULL;
   if (!resp || !cJSON_IsString(status) || strcmp(status->valuestring, "ok") != 0)
   {
   ...
   cJSON *ready = cJSON_GetObjectItemCaseSensitive(resp, "ready");
   cJSON *surfaces = cJSON_GetObjectItemCaseSensitive(resp, "surfaces_with_data");
   cJSON *min_rows = cJSON_GetObjectItemCaseSensitive(resp, "min_rows_required");

   printf("Calibration readiness\n");
   printf("  ready:             %s\n", cJSON_IsTrue(ready) ? "yes" : "no");
   printf("  surfaces with data:%d\n", cJSON_IsNumber(surfaces) ? (int)surfaces->valuedouble : 0);
   printf("  min rows required: %d\n", cJSON_IsNumber(min_rows) ? (int)min_rows->valuedouble : 200);
   cJSON_Delete(resp);
}
...
static void kb_cmd_demote(app_ctx_t *ctx, int argc, char **argv)
{
   static const char *bool_flags[] = {"dry-run", "report", NULL};
   opt_parsed_t opts;
   opt_parse(argc, argv, bool_flags, &opts);
   int json_out = ctx && ctx->json_output;

   char *raw = kb_client_demote_check_json();
   if (!raw)
   {
      fprintf(stderr, "demote: no response from aimee-kb\n");
      return;
   }
   if (json_out)
   {
   ...
   {
      cJSON *msg = resp ? cJSON_GetObjectItemCaseSensitive(resp, "message") : NULL;
      fprintf(stderr, "demote: %s\n", cJSON_IsString(msg) ? msg->valuestring : "unknown error");
      cJSON_Delete(resp);
      return;
   }

   cJSON *candidates = cJSON_GetObjectItemCaseSensitive(resp, "candidates");
   cJSON *scored = cJSON_GetObjectItemCaseSensitive(resp, "scored");
   cJSON *would_demote = cJSON_GetObjectItemCaseSensitive(resp, "would_demote");
   cJSON *enabled = cJSON_GetObjectItemCaseSensitive(resp, "demotion_enabled");
   cJSON *by_kind = cJSON_GetObjectItemCaseSensitive(resp, "by_kind");

   printf("Demotion dry-run\n");
   printf("  demotion_enabled:  %d\n", cJSON_IsNumber(enabled) ? (int)enabled->valuedouble : 0);
   printf("  candidates:        %d\n",
          cJSON_IsNumber(candidates) ? (int)candidates->valuedouble : 0);
   printf("  scored:            %d\n", cJSON_IsNumber(scored) ? (int)scored->valuedouble : 0);
   printf("  would demote:      %d\n",
          cJSON_IsNumber(would_demote) ? (int)would_demote->valuedouble : 0);

   if (cJSON_IsArray(by_kind) && cJSON_GetArraySize(by_kind) > 0)
   {
      printf("\n  By kind:\n");
      cJSON *entry;
      cJSON_ArrayForEach(entry, by_kind)
      {
         cJSON *kind = cJSON_GetObjectItemCaseSensitive(entry, "kind");
         cJSON *ks = cJSON_GetObjectItemCaseSensitive(entry, "scored");
         cJSON *kb = cJSON_GetObjectItemCaseSensitive(entry, "would_demote");
         cJSON *p10 = cJSON_GetObjectItemCaseSensitive(entry, "p10");
         printf("    %-20s scored=%d would_demote=%d p10=%.3f\n",
                cJSON_IsString(kind) ? kind->valuestring : "?",
                cJSON_IsNumber(ks) ? (int)ks->valuedouble : 0,
                cJSON_IsNumber(kb) ? (int)kb->valuedouble : 0,
                cJSON_IsNumber(p10) ? p10->valuedouble : 0.0);
      }
   }
   cJSON_Delete(resp);
}
...
static char *bandit_slurp_file(const char *path)
{
...
static void kb_cmd_bandit(app_ctx_t *ctx, int argc, char **argv)
{
   static const char *bool_flags[] = {"export", NULL};
   opt_parsed_t opts;
   opt_parse(argc, argv, bool_flags, &opts);
   int json_out = ctx && ctx->json_output;

   int do_export = opt_get_flag(&opts, "export");
   const char *record_path = opt_get(&opts, "record-replay");
   const char *record_dp = opt_get(&opts, "decision-point");

   if (record_path)
   {
      if (!record_dp || !record_dp[0])
      {
         fprintf(stderr, "usage: aimee kb bandit --record-replay <file.json> "
                         "--decision-point <name>\n");
         return;
      }
      char *result = bandit_slurp_file(record_path);
      if (!result)
      {
         fprintf(stderr, "bandit: cannot read %s\n", record_path);
         return;
      }
      char *resp = kb_client_bandit_replay_record_json(record_dp, result);
      free(result);
      if (json_out && resp)
      {
         puts(resp);
      ...
      {
         const char *aid = cJSON_GetStringValue(cJSON_GetObjectItem(root, "artifact_id"));
         printf("recorded benchmark_trace %s for decision_point=%s\n", aid ? aid : "?", record_dp);
      }
      else
      {
         const char *msg = root ? cJSON_GetStringValue(cJSON_GetObjectItem(root, "message")) : NULL;
         fprintf(stderr, "bandit replay-record: %s\n", msg ? msg : "unknown error");
      }
      cJSON_Delete(root);
      return;
   }
   ...
   {
      fprintf(stderr,
              "usage: aimee kb bandit --export\n"
              "       aimee kb bandit --record-replay <file.json> --decision-point <name>\n");
      return;
   }

   char *raw = kb_client_bandit_export_json();
   if (!raw)
   {
      fprintf(stderr, "bandit: no response from aimee-kb\n");
      return;
   }
   if (json_out)
   {
...
static const subcmd_t kb_subcmds[] = {
    ...
    {"import", "Import a JSON KB export: <file.json> [--workspace NAME] [--dry-run]",
     kb_cmd_import},
    {"calibrate", "Check calibration readiness (--check-readiness)", kb_cmd_calibrate},
    {"demote", "Demotion dry-run report (--dry-run --report)", kb_cmd_demote},
    {"bandit",
     "Export bandit arm decisions (--export) or record replay attribution "
     "(--record-replay FILE --decision-point NAME)",
     kb_cmd_bandit},
