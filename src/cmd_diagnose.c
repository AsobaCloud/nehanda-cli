...
void cmd_diagnose(app_ctx_t *ctx, int argc, char **argv)
{
   ...
   else if (strcmp(sub, "hypothesize") == 0)
   {
      if (argc < 2)
   ...
   else if (strcmp(sub, "evidence") == 0)
   {
      if (argc < 4)
         fatal(
             "diagnose evidence <diag_id> <hypothesis_id> <for|against> <content> [rank] [source]");
      int diag_id = atoi(argv[0]);
   ...
   else if (strcmp(sub, "investigate") == 0)
   {
      ...
      for (int i = 0; i < cap; i++)
      {
         ...
         if (!parallel)
         {
            ...
            char cmd[2048];
            snprintf(cmd, sizeof(cmd), "\"%s\" delegate investigator --persona engineer %s",
                     aimee_bin, prompt);
            printf("\n[H%d] Investigating: %s\n", i + 1, hyps[i].content);
            fflush(stdout);
            (void)system(cmd);
         ...
         else
         {
            ...
            if (pid == 0)
            {
               ...
               execlp(aimee_bin, "aimee", "delegate", "investigator", "--persona", "engineer",
                      prompt, (char *)NULL);
               _exit(127);
            }
            pids[n_pids++] = pid;
