/* test_workflow_session.c: unit tests for templated workflow sessions */
#include <assert.h>
#include <stdio.h>
#include <string.h>
#include <unistd.h>
...
{
   char dir[512];
   snprintf(dir, sizeof(dir), "%s/session_templates", root);
...
{
   char tmpdir[512];
   snprintf(tmpdir, sizeof(tmpdir), "%s/aimee-test-workflow-dir-XXXXXX", platform_tmpdir());
   assert(mkdtemp(tmpdir) != NULL);
   write_template(tmpdir);

   char tmpdb[512];
   snprintf(tmpdb, sizeof(tmpdb), "%s/aimee-test-workflow-db-XXXXXX", platform_tmpdir());
   ...
   int id = 0;
   char err[256] = "";
   assert(db1_workflow_session_create(tmpdir, "code-review", "review", assignments, &id, err,
                                      sizeof(err)) == 0);
   ...
   char *prompt = NULL;
   char *context = NULL;
   assert(db1_workflow_session_get(id, &info, &prompt, &context, err, sizeof(err)) == 0);
   ...
   assert(db1_workflow_session_advance(id, "human", "Please check auth flows first", &info, &prompt,
                                       err, sizeof(err)) == 0);
   assert(strcmp(info.status, "paused") == 0);
   assert(strstr(prompt, "Recent context") != NULL);
   free(prompt);

   assert(db1_workflow_session_advance(id, "claude-1", "I found one correctness issue", &info,
                                       &prompt, err, sizeof(err)) == 0);
   assert(strcmp(info.status, "active") == 0);
   assert(strcmp(info.expected_agent, "gemini") == 0);
   free(prompt);

   assert(db1_workflow_session_advance(id, "gemini", "I found a second issue", &info, &prompt, err,
                                       sizeof(err)) == 0);
   assert(strcmp(info.expected_agent, "claude-2") == 0);
   assert(strstr(prompt, "independent analysis") == NULL);
   free(prompt);

   assert(db1_workflow_session_advance(id, "claude-2", "I will fix both", &info, &prompt, err,
                                       sizeof(err)) == 0);
   assert(strcmp(info.expected_agent, "claude-1") == 0);
   free(prompt);

   assert(db1_workflow_session_advance(id, "claude-1", "Looks fixed", &info, &prompt, err,
                                       sizeof(err)) == 0);
   free(prompt);
   assert(db1_workflow_session_advance(id, "gemini", "Agreed", &info, &prompt, err, sizeof(err)) ==
          0);
...
static void test_wrong_agent_rejected(void)
{
   ...
   workflow_session_info_t info;
   char *prompt = NULL;
   assert(db1_workflow_session_advance(id, "gemini", "out of order", &info, &prompt, err,
                                       sizeof(err)) == -1);
...
static void test_list_and_json(void)
{
   ...
   int n = 0;
   char err[256] = "";
   assert(db1_workflow_session_list(&rows, &n, err, sizeof(err)) == 0);
   assert(n == 0);
   free(rows);
   ...
   int id1 = 0;
   assert(db1_workflow_session_create(tmpdir, "code-review", "review-a", a1, &id1, err,
                                      sizeof(err)) == 0);
   cJSON_Delete(a1);

   cJSON *a2 = make_assignments();
   int id2 = 0;
   assert(db1_workflow_session_create(tmpdir, "code-review", "review-b", a2, &id2, err,
                                      sizeof(err)) == 0);
   cJSON_Delete(a2);

   /* Pause the second one and check list sees both with correct status */
   assert(db1_workflow_session_pause(id2, "manual", err, sizeof(err)) == 0);
   ...
   cJSON *js = db1_workflow_session_info_to_json(&rows[0], "prompt-text", "ctx-text");
   assert(js != NULL);
   ...
   js = db1_workflow_session_info_to_json(&rows[1], NULL, NULL);
...
static void test_current_by_channel_prefers_active(void)
{
   ...
   int paused_id = 0;
   assert(db1_workflow_session_create(tmpdir, "code-review", "shared", first, &paused_id, err,
                                      sizeof(err)) == 0);
   cJSON_Delete(first);
   assert(db1_workflow_session_pause(paused_id, "manual", err, sizeof(err)) == 0);

   cJSON *second = make_assignments();
   int active_id = 0;
   assert(db1_workflow_session_create(tmpdir, "code-review", "shared", second, &active_id, err,
                                      sizeof(err)) == 0);
   cJSON_Delete(second);
   ...
   char *context = NULL;
   int current_id = 0;
   assert(db1_workflow_session_find_current_by_channel("shared", &current_id, err, sizeof(err)) ==
          0);
   assert(db1_workflow_session_get(current_id, &info, &prompt, &context, err, sizeof(err)) == 0);
   ...
   prompt = NULL;
   context = NULL;
   assert(db1_workflow_session_find_current_by_channel("missing", &current_id, err, sizeof(err)) ==
          -1);
...
int main(void)
{
   ...
   printf("test_workflow_session: all tests passed\n");
   return 0;
}
