/* test_curator_index_code_unit.c: smoke test for the code_unit vector indexer.
 * Exercises the public drain entry under the sqlite shim (no seeded code_unit
 * artifacts -> returns 0 without crashing). */
#include <assert.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>

#include <sqlite3.h>

#include "aimee.h"
#include "db2_test_shim.h"
#include "../kb/kb_curator_index_code_unit.h"

/* Stub the heavy embed + vector deps the handler references but this test never
 * reaches (the handler returns early with no seeded code_unit artifacts). */
int memory_embed_text(const char *text, const char *command, float *out, int max_dim)
{
   (void)text;
   (void)command;
   (void)out;
   (void)max_dim;
   return 0;
}
int pgvec_curator_code_unit_upsert(int64_t point_id, const float *intent_vec,
                                   const float *signature_vec, const float *body_vec, int dim,
                                   const char *artifact_id, const char *file_path,
                                   const char *def_kind, const char *signature,
                                   const char *body_hash, const char *payload_json)
{
   (void)point_id;
   (void)intent_vec;
   (void)signature_vec;
   (void)body_vec;
   (void)dim;
   (void)artifact_id;
   (void)file_path;
   (void)def_kind;
   (void)signature;
   (void)body_hash;
   (void)payload_json;
   return 0;
}

static void test_seeded_commits(void)
{
   /* Regression: the proposed-code_unit SELECT must use the real `payload` column
    * (not `payload_json`); a seeded proposed code_unit must be found + committed. */
   db2_test_shim_open();
   sqlite3 *db = (sqlite3 *)db2_test_shim_handle();
   assert(db != NULL);
   assert(sqlite3_exec(
              db,
              "INSERT INTO artifacts (id,kind,state,payload)"
              " VALUES "
              "('seed1','code_unit','proposed','{\"summary\":\"x\",\"domain_concepts\":[\"c\"]}')",
              NULL, NULL, NULL) == SQLITE_OK);
   int rc = kb_curator_index_code_unit_one(NULL);
   assert(rc == 1);
   sqlite3_stmt *st;
   assert(sqlite3_prepare_v2(db, "SELECT state FROM artifacts WHERE id='seed1'", -1, &st, NULL) ==
          SQLITE_OK);
   assert(sqlite3_step(st) == SQLITE_ROW);
   const char *state = (const char *)sqlite3_column_text(st, 0);
   assert(state && strcmp(state, "committed") == 0);
   sqlite3_finalize(st);
   db2_test_shim_close();
   printf("  index_code_unit commits a seeded proposed code_unit OK\n");
}

int main(void)
{
   db2_test_shim_open();
   int rc = kb_curator_index_code_unit_one(NULL);
   assert(rc == 0);
   db2_test_shim_close();
   printf("  index_code_unit graceful on empty/shim OK\n");
   test_seeded_commits();
   printf("curator_index_code_unit: all tests passed\n");
   return 0;
}
