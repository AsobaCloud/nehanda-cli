   ...
   if (pre_hash && pre_hash[0] && cfg->count > 0)
   {
      ...
      if (eidx >= 0 && ents[eidx].step_results[0])
      {
         for (int i = 0; i < cfg->count; i++)
         {
            int saved_rc = -1;
            if (step_state[i] == 0 &&
                verify_state_step_result_lookup(ents[eidx].step_results, cfg->steps[i].name, &saved_rc) &&
                saved_rc == 0)
            {
               contexts[i].rc = 0;
                 ...
                 size_t msg_len)
{
   ...
   if (entries[idx].step_results[0])
   {
      ...
      {
         int rc = -1;
         if (!verify_state_step_result_lookup(entries[idx].step_results, cfg.steps[i].name, &rc) || rc != 0)
         {
            if (msg_buf)
   ...
   if (is_async && server_ctx)
   {
      ...
      if (!coord->has_changes && coord->file_hash && coord->file_hash[0] && cfg.count > 0)
      {
         ...
         if (eidx >= 0 && ents[eidx].step_results[0])
         {
            ...
            {
               int saved_rc = -1;
               if (verify_state_step_result_lookup(ents[eidx].step_results, cfg.steps[i].name, &saved_rc) &&
                   saved_rc == 0)
               {
