#!/usr/bin/env python3
"""AIME reasoning benchmark runner.

Measures competition math reasoning on AIME problems.
Answers are integers from 0 to 999, graded by numeric extraction.

Usage:
  python3 benchmarks/reasoning/bench_aime.py \\
    --max-cases 50 --output benchmarks/results/aime_model_only_direct.json

Dataset: data/aime/test.jsonl or data/aime/aime_test.jsonl
...
# Match integers in a string (including negative), no decimals for AIME.
_INT_RE = re.compile(r"-?\d+")
# Match \boxed{...} for AIME answer extraction.
_BOXED_RE = re.compile(r"\\boxed\{(-?\d+)\}")


def _extract_gold(answer: str) -> str:
    """Extract the gold integer from an AIME answer field.
    
    AIME answers are single integers 0-999. The answer field may contain
    the boxed result or the plain integer.
    """
    m = _BOXED_RE.search(answer)
    if m:
        return m.group(1).strip()
    nums = _INT_RE.findall(answer)
    return nums[-1] if nums else ""


def _extract_predicted(response: str) -> str:
    """Extract the predicted integer from the agent response.
    
    Prefer \boxed{...} value; fall back to the last integer found.
    """
    m = _BOXED_RE.search(response)
    if m:
        return m.group(1).strip()
    nums = _INT_RE.findall(response)
    return nums[-1] if nums else ""


def _ints_equal(a: str, b: str) -> bool:
    """Compare two integer strings for equality (0-999 range)."""
    try:
        return int(a) == int(b)
...
def _load_dataset(data_dir: Path, max_cases: int) -> list[dict[str, Any]]:
    """Load AIME dataset from standard locations."""
    candidates = [
        data_dir / "aime" / "test.jsonl",
        data_dir / "aime" / "aime_test.jsonl",
    ]
    for path in candidates:
        if path.exists():
            ...
            return items
    canonical_url = "https://artofproblemsolving.com/wiki/index.php/AMC_history"
    raise FileNotFoundError(
        f"AIME dataset not found. Looked in: {[str(c) for c in candidates]}\n"
        f"Download AIME problems and place in data/aime/.\n"
        f"See: {canonical_url}"
    )
    ...
    for item in items:
        ...
        if _FAKE:
            ...
            user_prompt = (
                ...
                "Your final answer is an integer from 0 to 999. "
                "Put it in \\boxed{}.\n\n"
                + question
            )
        ...
        correct = _ints_equal(predicted, gold) if not _FAKE else False
        if correct:
            n_correct += 1
