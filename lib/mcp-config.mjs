...
/**
 * Find which mcp.json file a server is defined in (project-local first).
 * Returns { server, path } or null.
 */
export function getMcpServerSource(serverName, projectRoot) {
  const candidates = candidatePaths(projectRoot)
  for (const filePath of candidates) {
    const result = parseMcpFile(filePath)
    if (result && result.servers[serverName]) {
      return { server: result.servers[serverName], path: filePath }
    }
  }
  return null
}
...
/**
 * Set (or clear) an environment variable for an MCP server.
 * Updates the mcp.json file where the server is defined and reloads.
 *
 * @param {string} serverName  - Server name
 * @param {string} key         - Env var name (e.g. ASOBA_API_KEY)
 * @param {string} value       - Value to set (empty string clears the key)
 * @param {string} projectRoot - Project working directory
 * @returns {{ ok: boolean, path?: string, key?: string, set?: boolean, error?: string }}
 */
export function updateMcpServerEnv(serverName, key, value, projectRoot) {
  const candidates = candidatePaths(projectRoot)
  ...
  for (const filePath of candidates) {
    ...
    if (result && result.servers[serverName]) {
      ...
      const srv = raw.mcpServers[serverName]
      if (!srv.env) srv.env = {}
      if (value === '') {
        delete srv.env[key]
      } else {
      ...
      return { ok: true, path: filePath, key, set: value !== '' }
    }
  }

  return { ok: false, error: `Server "${serverName}" not found in any mcp.json` }
}
