...
import { applyEnergyNormalization } from './energy-middleware.mjs'
...
// Dynamically load declarative tools from the 'tools' directory
const TOOLS_CONFIG_DIR = path.join(path.dirname(new URL(import.meta.url).pathname), 'tools')
export const dynamicTools = new Map()

export const EXPLORE_ONLY_TOOLS = new Set(['Read', 'Glob', 'Grep'])
const PLANNING_BLOCKED_TOOLS = new Set(['Write', 'Edit', 'Bash', 'NotebookEdit'])

try {
  if (fs.existsSync(TOOLS_CONFIG_DIR)) {
    const files = fs.readdirSync(TOOLS_CONFIG_DIR)
    for (const file of files) {
      if (file.endsWith('.json')) {
        const filePath = path.join(TOOLS_CONFIG_DIR, file)
        const config = JSON.parse(fs.readFileSync(filePath, 'utf8'))
        dynamicTools.set(config.name, config)

        if (config.phases?.explore_only) {
          EXPLORE_ONLY_TOOLS.add(config.name)
        }
        if (config.phases?.planning_blocked) {
          PLANNING_BLOCKED_TOOLS.add(config.name)
        }
      }
    }
  }
} catch (err) {
  console.error('Failed to load declarative tools:', err)
}
...
export async function executeBuiltinTool(db, ctx, toolName, toolInput, io) {
  ...
  try {
    switch (toolName) {
      ...
      default:
        if (dynamicTools.has(toolName)) {
          return await executeDynamicScriptTool(cwd, toolName, toolInput)
        }
...
// ─── Dynamic Script Tool Executor ───────────────────────────────

async function executeDynamicScriptTool(cwd, toolName, input) {
  const config = dynamicTools.get(toolName)
  if (!config) return { content: `Tool ${toolName} config not found`, is_error: true }

  let scriptPath = config.execution.script
  if (!path.isAbsolute(scriptPath)) {
    scriptPath = path.join(path.dirname(new URL(import.meta.url).pathname), '..', scriptPath)
  }

  // Override dynamically using environment variable (e.g. AUDIT_CODE_INTEGRITY_SCRIPT for AuditCodeIntegrity)
  const envKey = `${toolName.replace(/([A-Z])/g, '_$1').toUpperCase()}_SCRIPT`.replace(/^_/, '')
  const resolvedScriptPath = process.env[envKey] || scriptPath

  if (!fs.existsSync(resolvedScriptPath)) {
    return { content: `${toolName}: script not found at ${resolvedScriptPath}.`, is_error: true }
  }

  // Parse arguments template
  const spawnArgs = []
  ...
  for (let i = 0; i < templateArgs.length; i++) {
    ...
    if (arg.startsWith('{{') && arg.endsWith('}}')) {
      ...
      const val = input[key]
      if (val !== undefined) {
        spawnArgs.push(String(val))
      }
    } else {
      // Check if next arg is a placeholder, and if its value is undefined, skip both the flag and the placeholder
      const nextArg = templateArgs[i + 1]
      if (nextArg && nextArg.startsWith('{{') && nextArg.endsWith('}}')) {
        const key = nextArg.slice(2, -2)
  ...
  const cmd = config.execution.runtime || 'python3'
  const args = [resolvedScriptPath, ...spawnArgs]
  const timeoutMs = Math.min(
    input?.timeout || config.execution.default_timeout || 120_000,
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (code !== 0 && code !== 1) {
        resolve({ content: stderr || `${toolName} exited with code ${code}`, is_error: true })
        return
      }
      if (code === 1 && !stdout.trim()) {
        resolve({ content: stderr || `${toolName} execution error (exited with code 1 and empty output)`, is_error: true })
        return
      }
...
async function toolMcpInvoke(ctx, toolName, toolInput) {
  ...
  const result = await callTool(srv, mcpToolName, toolInput || {})

  // Pass energy payloads through ODSE normalization if OEM is recognized.
  // On any failure applyEnergyNormalization returns the original content unchanged.
  if (!result.is_error && result.content) {
    const { content } = await applyEnergyNormalization(serverName, result.content)
...
export function anthropicToolDefinitions() {
  ...
  for (const [name, config] of dynamicTools) {
    tools.push({
      name: config.name,
...
/** In-memory cache: serverName → { tools, ts } */
const _mcpToolCache = new Map()
const MCP_CACHE_TTL_MS = 60_000   // re-discover after 1 minute

/**
 * Connect to all configured MCP servers, call tools/list on each, and return
 * tool definitions in Anthropic format.  Results are cached per-server for
 * MCP_CACHE_TTL_MS to avoid spawning child processes on every turn.
 *
 * Tool names are namespaced as  mcp__<server>__<tool>  so they route through
 * toolMcpInvoke() in executeBuiltinTool() without any extra plumbing.
 *
 * @param {object} settings  - Effective settings object (settings.mcp_servers used)
 * @returns {Promise<Array>}   Anthropic-format tool definitions
 */
export async function getMcpToolDefinitions(settings) {
  const serverConfigs = settings?.mcp_servers
  if (!serverConfigs || typeof serverConfigs !== 'object') return []
  ...
  for (const name of names) {
    ...
    const cached = _mcpToolCache.get(name)
    if (cached && now - cached.ts < MCP_CACHE_TTL_MS) {
      allTools.push(...cached.tools)
      continue
    }

    try {
      const srv = getMcpServer(name, config)
      const resp = await srv.request('tools/list', {})
      if (resp.error) continue

      const serverTools = (resp.result?.tools || []).map(t => ({
        name: `mcp__${name}__${t.name}`,
        description: `[MCP:${name}] ${t.description || t.name}`,
        input_schema: t.inputSchema || { type: 'object', properties: {} },
      }))

      _mcpToolCache.set(name, { tools: serverTools, ts: now })
      allTools.push(...serverTools)
    } catch {
      // Server unreachable — skip silently, don't break the turn
    }
  }
...
/**
 * Invalidate the MCP tool cache for one server (or all if no name given).
 * Called by /mcp reload.
 */
export function invalidateMcpToolCache(serverName) {
  if (serverName) _mcpToolCache.delete(serverName)
  else _mcpToolCache.clear()
}
