...
/**
 * Build tool-call format instructions for providers that cannot receive
 ...
 *
 * IMPORTANT: We intentionally do NOT use <tool_call> XML tags here. The
 * Nehanda vLLM deployment runs --tool-call-parser qwen3_xml, which registers
 * <tool_call> as a stop/intercept token. If the model emits that token, vLLM
 * terminates generation and hands off to its native parser — which then returns
 * nothing (because the plain-text path never populates message.tool_calls).
 * The result is a truncated response with only the thinking trace in content.
 *
 * Instead we use [TOOL_CALL]...[/TOOL_CALL] as the delimiter. Probing confirmed
 * the model produces complete, correctly-structured output with this marker.
 * parseXmlToolCalls() matches this same delimiter pattern.
 *
...
function buildXmlToolInstructions(tools, phase) {
  ...
  // Show just the format + the 3 most commonly needed tools as concrete examples.
  // The model doesn't need all 21 schemas — it needs the format and tool names.
  ...
  return `\n\n# Tool Calling Format\n\nTo call a tool, emit a [TOOL_CALL] block containing a JSON object with "name" and "arguments" keys. Examples:\n\n${examples}\n\nAll available tools: ${allNames}\n\nRules: one tool call per response. After all tool use is complete, write your final answer as plain text.`
}
...
/**
 * Rescue parser for models that emit [TOOL_CALL]...[/TOOL_CALL] blocks
 * (Nehanda/Qwen vLLM path). The format is a JSON object with "name" and
 * "arguments" keys inside the delimiter pair:
 *
 *   [TOOL_CALL]
 *   {"name": "Bash", "arguments": {"command": "ls /tmp"}}
 *   [/TOOL_CALL]
 *
 * Why not <tool_call> XML? The Nehanda vLLM deployment runs
 * --tool-call-parser qwen3_xml, which registers <tool_call> as a stop token.
 * Emitting it causes vLLM to terminate generation mid-response and hand off
 * to its native parser, returning only the thinking trace in content.
 * [TOOL_CALL] bypasses that interception entirely.
 *
 * Safety: results are gated on `name` matching a known tool from filteredTools
 * to prevent false-positive promotion of ordinary JSON in prose responses.
 *
 * Given: model response text containing one or more [TOOL_CALL] blocks
 * When: parseXmlToolCalls(text, knownToolNames) is called
...
function parseXmlToolCalls(text, knownToolNames = null) {
  ...
  while ((blockMatch = blockRe.exec(text)) !== null) {
    ...
    // Gate on known tool names when provided — prevents false-positive tool
    // execution from ordinary JSON blobs in conversational responses.
...
async function runOpenAICompatPhase(db, rt, io, hookRt, provider, baseUrl, apiKey, allTools, injectedTools, phase) {
  ...
  for (;;) {
    ...
    // nehandaMlProxy which strips tools/tool_choice), inject tool-call format
    ...
    // NOTE: We use [TOOL_CALL] not <tool_call> — see buildXmlToolInstructions
    // for the reason (vLLM stop-token interception).
    ...
    // §1.2 Late Injection Pattern: append the XML tool-call directive to the
    // final user message so it is the last tokens the model attends to before
    // generating its response. This defeats recency bias on Qwen/vLLM models
    // where system-prompt instructions are overridden by RLHF conversational defaults.
    let contextMessages = filteredMessages
    if (omitToolChoice(provider) && currentPhase !== 'planning') {
      const lastUserIdx = contextMessages.map(m => m.role).lastIndexOf('user')
      if (lastUserIdx !== -1) {
        const lateDirective = '\n\n[SYSTEM DIRECTIVE: If you need to read files, run shell commands, or inspect the repo, you MUST emit a [TOOL_CALL] block NOW. Do not explain what you plan to do in plain text. Output the [TOOL_CALL] block directly.]'
        const msg = contextMessages[lastUserIdx]
        const newContent = typeof msg.content === 'string'
          ? msg.content + lateDirective
          : msg.content
        contextMessages = [
          ...contextMessages.slice(0, lastUserIdx),
          { ...msg, content: newContent },
          ...contextMessages.slice(lastUserIdx + 1),
        ]
      }
    }

    const messages = [{ role: 'system', content: fullSystemPrompt }, ...contextMessages]

    let assistantBlocks, bufferedText = null, finishReason = null
    try {
      const out = await streamOpenAIChatCompletion({ baseUrl, apiKey, model: rt._model, messages, tools: filteredTools, io, bufferOutput: true, numCtx: rt.settings?.model_config?.num_ctx, omitToolChoice: omitToolChoice(provider), useStream: !omitToolChoice(provider) })
      ...
      // Priority: [TOOL_CALL] blocks (Nehanda/Qwen format) then JSON code-fence blocks.
      // knownToolNames gates the parsers against false-positive promotion of
      // ordinary JSON in prose responses.
    ...
    // IMPORTANT: only treat 'stop'/'length' as a terminal signal when there are
    // no tool calls to execute. If the rescue parsers found tool_use blocks,
    // we must execute them even though finish_reason is 'stop' — backends like
    // Nehanda/vLLM emit finish_reason:'stop' even when the model called a tool
    // in its content (they never return 'tool_calls' for text-embedded calls).
    if (!toolUses.length) {
      return displayText || bufferedText
    }
    if (finishReason === 'length') {
      // Truncated mid-generation — tool call may be incomplete; safer to bail.
      return displayText || bufferedText
    }
