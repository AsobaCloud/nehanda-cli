...
export async function streamOpenAIChatCompletion({ baseUrl, apiKey, model, messages, tools, io, maxTokens = 8192, bufferOutput = true, num_ctx: numCtx, omitToolChoice: skipToolChoice = false, useStream = true }) {
  const url = `${String(baseUrl).replace(/\/$/, '')}/chat/completions`
  const body = { model, messages, stream: useStream, max_tokens: maxTokens }
  ...
  // Non-streaming path: Lambda/proxy returns a single JSON completion object.
  // Used for providers like Nehanda whose Lambda Function URL buffers the full
  // response rather than forwarding SSE chunks.
  if (!useStream) {
    const text = await res.text()
    let json
    try { json = JSON.parse(text) } catch { throw new Error(`OpenAI-compat: non-JSON response: ${text.slice(0, 500)}`) }
    const content = json.choices?.[0]?.message?.content || ''
    const strippedText = stripThinkingTraces(content)
    if (!bufferOutput && strippedText) { if (io.spinner) io.spinner.stop(); io.write(strippedText) }
