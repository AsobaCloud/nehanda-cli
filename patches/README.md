...
| `015-openai-chat-hybrid-dispatch.patch` | Hybrid dispatch for `/v1/chat/completions`: use `agent_execute_with_tools` when client provides tools, otherwise `agent_execute`. 
| `015-worktree-untracked-sync-and-tool-sanitize.patch` | Worktree untracked sync and tool sanitization fixes 
| `016-delegate-openai-xml-tool-call-rescue.patch` | XML fallback rescue parsing in delegate_openai.c: if no native JSON tool_calls were parsed, scan content for `tool_calls` / <invoke> / XML blocks and rescue them. 
| `017-streaming-tool-loop.patch` | Streaming tool loop with `agent_run_with_tools` in openai_chat.c: sets `agent_set_ingress_source("openai-ingress")` before tool execution in streaming handlers (`chat_stream_handler`, `completion_stream_handler`). 
| `018-non-streaming-tool-loop.patch` | Non-streaming tool loop with `agent_run_with_tools` in openai_chat.c: extends patch 017 to non-streaming handlers (`run_completion` and `responses_handler`) so tool calling works when `stream: false`. |
