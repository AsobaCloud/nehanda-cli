...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --endpoint)
            if [ "$#" -lt 2 ]; then
                error "--endpoint requires a value"
                exit 1
            fi
...
if [ -z "$TARGET_VALUE" ] && [ -n "${AIMEE_LOCAL_DELEGATE_ENDPOINT:-}" ]; then
    TARGET_KIND="endpoint"
    TARGET_VALUE="$AIMEE_LOCAL_DELEGATE_ENDPOINT"
fi

if [ -z "$TARGET_VALUE" ] && [ -n "${AIMEE_LOCAL_DELEGATE_HOST:-}" ]; then
    TARGET_KIND="host"
    TARGET_VALUE="$AIMEE_LOCAL_DELEGATE_HOST"
fi
...
detect_runtime() {
    ...
    probe_target() {
        ...
        # If a full /v1 endpoint was supplied, only probe the OpenAI-style path.
        if [[ "$base_url" == */v1 ]]; then
            probe_base_url "$base_url" "$base_url/models" "llama-cpp" "$base_url" && return 0
            return 1
        fi

        # Otherwise probe both styles on the same base URL first.
        probe_base_url "$base_url" "$base_url/api/tags" "ollama" "$base_url/v1" && return 0
        probe_base_url "$base_url" "$base_url/v1/models" "llama-cpp" "$base_url/v1" && return 0
        return 1
    }
    ...
    probe_host_target() {
        ...
        # Bare hosts should keep the existing localhost UX sane by probing the
        # common Ollama and llama.cpp ports.
        if [[ "$host" != *:* ]]; then
            probe_target "http://${host}:11434" && return 0
            probe_target "http://${host}:8080" && return 0
            return 1
        fi
    ...
    if [ -n "$TARGET_VALUE" ]; then
        if [ "$TARGET_KIND" = "host" ]; then
            if probe_host_target "$TARGET_VALUE"; then
                return 0
            fi
        else
            if probe_target "$TARGET_VALUE"; then
                return 0
            fi
        fi

        warn "Could not verify ${TARGET_KIND:-endpoint} '$TARGET_VALUE'; using it as a custom endpoint"
        if [[ "$TARGET_VALUE" == http://* || "$TARGET_VALUE" == https://* ]]; then
            ENDPOINT="${TARGET_VALUE%/}"
        else
            ENDPOINT="http://${TARGET_VALUE%/}"
        fi
        if [[ "$ENDPOINT" != */v1 ]]; then
...
handle_no_runtime() {
    ...
    echo "  2) Enter a custom host or OpenAI-compatible endpoint"
...
register_agent() {
    "$AIMEE_BIN" agent local "$AGENT_NAME" "$ENDPOINT" \
        --model "$MODEL" \
        --default
...
main() {
    ...
    if [ -n "$TARGET_VALUE" ]; then
        info "Configured target: ${TARGET_KIND:-endpoint} $TARGET_VALUE"
    fi
